<?php
	...
	$r = $con->dbquery("select concat('[',acct_code,'] ',description) as acct, acct_code from acctg_accounts where  LOCATE('$_POST[queryString]', description) > 0 and company = '$_SESSION[company]' LIMIT 10");
		...
		if(strlen($_POST['queryString']) > 0) {
				echo "<table width=100% cellpadding=5 cellspacing=0 onMouseOut=\"javascript: highlightTableRowVersionA(0);\">
					...
					while (list($acct_desc,$acct_code) = $r->fetch_array()) {
						...
						echo "<tr onMouseOver=\"javascript: highlightTableRowVersionA(this, '#e6f0fa');\" bgcolor='$bgC' onclick=\"pickAccountReport('$acct_code','$_POST[el]');\" style=\"cursor: pointer;\" >
