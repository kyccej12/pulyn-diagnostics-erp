<?php
	...
	class authenticate extends myDB {
		...
		function verify($uname,$pass,$comp) {
			if(!empty($uname) && !empty($pass)) {
				...
				if($res) {
					...
					if(!empty($uid)) {
						$this->storeSession($uid,$utype,$comp);
						return true;
					} else {
						return false;
					}
		...
		function storeSession($uid,$utype,$comp) {
	...
	if($auth->verify($_POST['txtname'],$_POST['txtpass'],"1") == true) {
		$URL = $HTTP_REFERER . "index.php";
	} else {
		$URL = $HTTP_REFERER . "login/index.php";
	}
