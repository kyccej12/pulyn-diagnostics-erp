<?php
	...
	$data = array();
	$datares = $con->dbquery("SELECT LPAD(prio,6,0) AS prio, LPAD(so_no,6,0) AS so, DATE_FORMAT(so_date,'%m/%d/%Y') AS sodate, `code`, `procedure`, CONCAT(b.lname,', ',b.fname,', ',b.mname) AS pname, b.gender, DATE_FORMAT(b.birthdate,'%m/%d/%Y') AS bday, FLOOR(DATEDIFF(so_date,b.birthdate)/364.25) AS age, compname, a.clinic, a.so_date,prio AS `priority`,b.birthdate, a.pid, CONCAT(c.fullname,', ',c.prefix) AS ex_by, CONCAT(d.fullname,', ',d.prefix) AS ev_by FROM peme a LEFT JOIN patient_info b ON a.pid = b.patient_id LEFT JOIN options_doctors c ON a.examined_by = c.id LEFT JOIN options_doctors d ON a.evaluated_by = d.id WHERE 1=1 AND a.examined_by > 0 and a.evaluated_by > 0;");
