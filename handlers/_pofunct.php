<?php
	...
	class myPO extends _init {
			...
			if($lock != 'Y') {
				switch($status) {
					case "Finalized":
						...
						$headerControls = $headerControls . "&nbsp;<a href=\"#\" class=\"topClickers\" onClick=\"javascript:parent.printPO('$po_no');\"><img src=\"images/icons/print.png\" width=16 height=16 border=0 align=\"absmiddle\">&nbsp;Print Purchase Order</a>&nbsp;";
					break;
					case "Cancelled":
						if($urights == "admin") {
				...
				$headerControls = $headerControls . "&nbsp;<a href=\"#\" class=\"topClickers\" onClick=\"javascript:parent.printPO('$po_no');\"><img src=\"images/icons/print.png\" width=16 height=16 border=0 align=\"absmiddle\">&nbsp;Re-Print Purchase Order</a>&nbsp;";
			}
			echo $headerControls;
		}
		...
		function updateHeaderAmt($po_no,$bid) {
			list($amt) = parent::getArray("select sum(ROUND(qty*cost,2)) as amount from po_details where po_no = '$po_no' and branch = '$bid';");
			parent::dbquery("update ignore po_header set amount = '0$amt' where po_no = '$po_no' and branch = '$bid';");
		}
