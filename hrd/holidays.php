	...
	<script>
		...
		$(document).ready(function() {
			...
			$('#itemlist').dataTable({
				...
				"bProcessing": true,
				"sAjaxSource": "listings.php?mod=natholidays&sid="+Math.random()+"",
				"aoColumns": [
				  { mData: 'id' },
				  { mData: 'type' },
				  { mData: 'xdate' },
		...
		function viewRecord() {
			...
			if(!UID) {
				...
				$.post("misc-data.php", { mod: "getNatHoliday", id: UID, sid: Math.random() }, function(ech) {
					$("#rid").val(ech['id']);
					$("#date").val(ech['xdate']);
					$("#type").val(ech['xtype']);
					...
					$("#record").dialog({title: "Holiday Occasion", width: 440, modal: true, resizable: false, buttons: { 
						"Save Changes": function() {
							if($("#type").val() != '') {
								$.post("misc-data.php", { mod: "updateNatHoliday", rid: $("#rid").val(), date: $("#date").val(), type: $("#type").val(), occasion: $("#occasion").val(), sid: Math.random() }, function() {
			...
			$("#record").dialog({title: "Holiday Occasion", width: 440, modal: true, resizable: false, buttons: {
				"Save Record": function() {
					...
					if(msg != "") { parent.sendErrorMessage(msg); } else {
						$.post("misc-data.php", { mod: "newNatHoliday", date: $("#date").val(), type: $("#type").val(), occasion: $("#occasion").val(), sid: Math.random() }, function() {
		...
		function deleteRecord() {
			...
			if(UID == "") {
				...
				if(confirm("Are you sure you want to delete this record?") == true) {
					...
					$.post("misc-data.php", { mod: "deleteNatHoliday", rid: UID, sid: Math.random() }, function() {
...
<div id="record" style="display: none;">
	<form name="fhrmHoliday" id="frmHoliday">
		<table width=100% cellspacing=2 cellpadding=0>
			<tr>
				<td class="bareThin" align=left width=40%>Holiday Type :</td>
				<td align=left>
					<select name="type" id="type" style="width: 80%; font-size: 11px; padding: 5px;">
						<option value="1">Legal/Regular</option>
