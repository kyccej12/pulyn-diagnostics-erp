<?php
	...
	if(isset($_REQUEST['queryString']) && $_REQUEST['queryString']!= "") {
		...
		$r = mysql_query("select item_code,description,unit_cost,unit from products_master where company = '$_SESSION[company]' and (locate('$_REQUEST[queryString]',description) > 0 or locate('$_REQUEST[queryString]',barcode) or locate('$_REQUEST[queryString]',item_code)) limit 10");
