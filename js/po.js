...
function savePOHeader() {
	...
	if(msg != "") {
		...
		$.post("po.datacontrol.php", { mod: "saveHeader", trace_no: $("#trace_no").val(), po_no: $("#po_no").val(), po_date: $("#po_date").val(), terms: $("#terms").val(), cid: $("#customer_id").val(), cname: $("#customer_name").val(), addr: $("#cust_address").val(), requested_by: $("#requested_by").val(), mrs: $("#mrs_no").val(), del_addr: $("#delivery_address").val(), date_needed: $("#date_needed").val(), remarks: $("#remarks").val(), sid: Math.random() },function(pono){
			$("#po_no").val(pono);
			parent.popSaver();
		},"html");
	}
   ...
   $.each(table.rows('.selected').data(), function() {
	...
	if(!arr[0]) {
		...
		if(confirm("Are you sure you want to remove this line entry?") == true) {
			$.post("po.datacontrol.php", { mod: "deleteLine", lid: arr[0], po_no: $("#po_no").val(), sid: Math.random() }, function() { redrawDataTable(); getTotals(); });
		}
	}
}
	...
	$.post("po.datacontrol.php", { mod: "check4print", po_no: po_no, sid: Math.random() }, function(data) { 
		if(data == "noerror") {
			if(confirm("Are you sure you want to finalize this Purchase Order?") == true) {
				$.post("po.datacontrol.php", { mod: "finalizePO", po_no: po_no, sid: Math.random() }, function() { parent.viewPO(po_no); });
			}
		} else {
			switch(data) {
...
function reopenPO() {
	if(confirm("Are you sure you want to set this document to active status?") == true) {
		$.post("po.datacontrol.php", { mod: "reopenPO", po_no:  $("#po_no").val(), sid: Math.random() }, function() {
			location.reload();
		});
	}
}
...
function cancelPO() {
	if(confirm("Are you sure you want to Cancel this document?") == true) {
		$.post("po.datacontrol.php", { mod: "cancel", po_no:  $("#po_no").val(), sid: Math.random() }, function(){
