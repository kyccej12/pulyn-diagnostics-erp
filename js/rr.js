...
function saveRRHeader() {
	...
	if(msg != "") {
		...
		$.post("rr.datacontrol.php", { mod: "saveHeader", trace_no: $("#trace_no").val(), rr_no: $("#rr_no").val(), rr_date: $("#rr_date").val(), cid: $("#customer_id").val(), cname: $("#customer_name").val(), addr: $("#cust_address").val(), recby: $("#received_by").val(), ino: $("#invoice_no").val(), idate: $("#invoice_date").val(), remarks: $("#remarks").val(), sid: Math.random() },function(data){
			$("#rr_no").val(data);
			parent.popSaver();
		});
	}
...
function loadPO() {
	$.post("rr.datacontrol.php", { mod: "loadPO", trace_no: $("#trace_no").val(), rr_no: $("#rr_no").val(), sid: Math.random() }, function() {
		getTotals();
		redrawDataTable();
   ...
   $.each(table.rows('.selected').data(), function() {
	...
	if(!arr[0]) {
		...
		if(confirm("Are you sure you want to remove this line entry?") == true) {
			$.post("rr.datacontrol.php", { mod: "deleteLine", lid: arr[0], rr_no: $("#rr_no").val(), sid: Math.random() }, function() { redrawDataTable(); getTotals(); });
		}
	}
}
...
function finalize() {
	$.post("rr.datacontrol.php", { mod: "check4print", rr_no: $("#rr_no").val(), sid: Math.random() }, function(data) { 
		if(data == "noerror") {
			if(confirm("Are you sure you want to finalize this Receiving Report?") == true) {
				$.post("rr.datacontrol.php", { mod: "finalizeRR", rr_no: $("#rr_no").val(), sid: Math.random() }, function() {
					parent.viewRR($("#rr_no").val());
				});
			}
		} else {
			switch(data) {
...
function reopenRR() {
	if(confirm("Are you sure you want to set this document to active status?") == true) {
		$.post("rr.datacontrol.php", { mod: "reopenRR", rr_no: $("#rr_no").val(), sid: Math.random() }, function() {
			location.reload();
		});
	}
}
...
function cancelRR() {
	if(confirm("Are you sure you want to Cancel this document?") == true) {
		$.post("rr.datacontrol.php", { mod: "cancel", rr_no:  $("#rr_no").val(), sid: Math.random() }, function(){
