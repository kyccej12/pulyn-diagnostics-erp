	...
	switch($_REQUEST['mod']) {
				...
				if($so['billed'] == 'Y' or $so['paid'] == 'Y') {
					...
					if($s[0] != '') {
						...
						while($sdRow = $sdQuery->fetch_array()) {
							$con->dbquery("INSERT IGNORE INTO or_details (doc_no,branch,so_no,so_date,pid,pname,paddr,`code`,`description`,unit,unit_price,is_special,qty,amount,discount,amount_due,trace_no) VALUES ('$docno','$bid','$sdRow[so_no]','$sdRow[so_date]','$sdRow[patient_id]','$sdRow[patient_name]','$sdRow[patient_address]','$sdRow[code]','$sdRow[description]','$sdRow[unit]','$sdRow[unit_price]','$sdRow[is_special]','$sdRow[qty]','$sdRow[amount]','$sdRow[discount]','$sdRow[amount_due]','$_POST[trace_no]');");
						}
		...
		case "finalize":
			...
			while($dRow = $dQuery->fetch_array()) {
				...
				$so = $con->dbquery("SELECT a.so_no AS so, b.code as parent_code, b.code, b.description AS `procedure`, a.physician, d.sample_type, d.container_type FROM so_header a LEFT JOIN so_details b ON a.trace_no = b.trace_no LEFT JOIN patient_info c ON a.patient_id = c.patient_id LEFT JOIN services_master d ON b.code = d.code WHERE a.status = 'Finalized' AND a.cstatus IN (2,4,12) AND d.with_subtests = 'N' AND d.category IN ('1','2','3') AND a.so_no = '$dRow[so_no]' UNION SELECT a.so_no AS so, e.parent as parent_code, e.code, e.description AS `procedure`, a.physician, f.sample_type, f.container_type FROM so_header a LEFT JOIN so_details b ON a.trace_no = b.trace_no LEFT JOIN patient_info c ON a.patient_id = c.patient_id LEFT JOIN services_master d ON b.code = d.code LEFT JOIN services_subtests e ON b.code = e.parent LEFT JOIN services_master f ON e.code = f.code WHERE a.status = 'Finalized' AND a.cstatus IN (2,4,12) AND d.with_subtests = 'Y' AND f.category IN ('1','2','3') AND a.so_no = '$dRow[so_no]';");
