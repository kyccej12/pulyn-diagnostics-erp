<?php
	...
	$res2 = getArray("select record_id, or_no, date_format(or_date,'%m/%d/%Y') as or_date, month, year, type, pay_type, check_no, if(check_date='0000-00-00','',date_format(check_date,'%m/%d/%Y')) as check_date, amount, received_by from payments where month='$_GET[month]' and year='$_GET[year]' and type='$_GET[type]' and file_id='$_GET[fileID]';");
	session_start();
?>
<html>
<head>
...
<script>
	...
	function savePay(submod) {
		...
		urem = prompt("Reason in updating this record:");
		if(urem == "") { parent.sendErrorMessage("Unable to update record as you have not indicated any reason as to why you wish to update this record."); return false; }
		if(urem == null) { return false; }
		
		$.post("tennant.datacontrol.php", { mod: "updatePayment", rid: $("#rid").val(), file_id: $("#file_id").val(), or_no: $("#or_no").val(), or_date: $("#or_date").val(), month: $("#month").val(), year: $("#year").val(), type: $("#type").val(), amount: $("#amount_due").val(), pay_type: $("#p_type").val(), check_no: $("#check_no").val(), check_date: $("#check_date").val(), recby: $("#receivedby").val(), urem: urem, sid: Math.random() },function() {
			alert("Record Successfully Saved...");
			parent.close_div2(); parent.showPayHistory(<?php echo $_GET['fileID']; ?>);
		});
...
<body bgcolor="#7f7f7f" leftmargin="0" bottommargin="0" rightmargin="0" topmargin="0">
 <table height="100%" width="100%" border="0" cellspacing="0" cellpadding="0" >
	...
	<input type="hidden" id = "rid" value="<?php echo $res2['record_id']; ?>">
	<tr>
						...
						<table border="0" cellpadding="0" cellspacing="0" width=100%>
									...
									<fieldset>
										<legend><span class="spandix">PAYMENT DETAILS:</span></legend>
											<table width=100% cellpadding=0 cellspacing=0 border=0>
												<tr><td class=spandix width=50%>OR # :</td><td><input type="text" id="or_no" class="nInput" value="<?php echo $res2['or_no']; ?>"  style="width: 60%" /></td></tr>
												<tr><td height=1></td></tr>
												<tr><td class=spandix>OR Date :</td><td><input type="text" id="or_date" class="nInput" style="width: 60%" value="<?php echo $res2['or_date']; ?>" /></td></tr>
												<tr><td height=1></td></tr>
												...
												<tr><td class=spandix>Payment Type:</td>
													<td>
														<select id="p_type" class="nInput" style="width: 60%;">
															<option value="Cash" <?php if($res2['pay_type'] == 'Cash') { echo "selected"; } ?>>Cash Payment</option>
															<option value="Check" <?php if($res2['pay_type'] == 'Check') { echo "selected"; } ?>>Check Payment</option>
														</select>
													</td>
												</tr>
												<tr><td height=1></td></tr>
												<tr><td class=spandix>Check # (For Check Payment Only) :</td><td><input type="text" id="check_no" value="<?php echo $res2['check_no']; ?>" class="nInput" style="width: 60%" /></td></tr>
												<tr><td height=1></td></tr>
												<tr><td class=spandix>Check Date (For Check Payment Only) :</td><td><input type="text" id="check_date"  value="<?php echo $res2['check_date']; ?>" class="nInput"   style="width: 60%" /></td></tr>
												<tr><td height=1></td></tr>
												<tr><td class=spandix>Payment Received By :</td><td><input type="text" id="receivedby" value="<?php echo $res2['received_by']; ?>" class="nInput" style="width: 60%" /></td></tr>
											</table>
									  </fieldset>
								</td>
							</tr>
							...
							<tr>
								<td align=center colspan=3>
									<button onClick="savePay();" class="buttonding" id="btn_rsv" style="width: 250px; <?php if($_SESSION['utype'] != 'admin') { echo "visibility: hidden;"; } ?>"><img src="images/icons/floppy.png" width=24 height=24 align=absmiddle />&nbsp;&nbsp;Save Changes to this Record</b></button>
								</td>
							</tr>
						</table>
					</td>
