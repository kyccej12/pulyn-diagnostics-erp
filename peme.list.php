				...
				buttons: [
					{
						...
						click: function() {
							if(confirm("Are you sure you want to assign this patient to the selected clinic?") == true) {
								$.post("src/sjerp.php", { mod: "assignToClinic", pid: pid, so_no: so_no, clinic: $("#assClinic").val(), sid: Math.random() }, function (){
									refreshList();
									dis.dialog("close");
								});
							}
	...
	<table width="100%" cellspacing="0" cellpadding="0" style="padding-left: 5px; margin-bottom: 2px;">
		<tr>
			<td>
				...
				<button class="ui-button ui-widget ui-corner-all" onClick="assignClinic();">
					<span class="ui-icon  ui-icon-transferthick-e-w "></span> Assign Clinic
				</button>
			...
			<!-- <td align=right style="padding-right:18px;">
				...
				<select name="displayType" class="gridInput" style="width: 250px; font-size: 11px;" onchange="javascript: changeDisplay(this.value);">
					...
					<option value="2" <?php if($_GET['displayType'] == 2) { echo "selected"; } ?>>For Doctor's Evaluation</option>
					<option value="3" <?php if($_GET['displayType'] == 3) { echo "selected"; } ?>>Completed</option>
				</select>
			</td> -->
		</tr>
	</table>
...
<div id="clinicAssignment" style="display: none;">
	<table width=100% cellpadding=5>
		<tr>
			<td class="spandix-l" width=35%>Clinic Assignment :</td>
			<td>
