<?php
	...
	if(isset($_GET['id']) && $_GET['id'] != "") { 
		$res = $con->getArray("select *, if(date_format(expiry_d8,'%m/%d/%Y')='00/00/0000','',date_format(expiry_d8,'%m/%d/%Y')) as expiry_d8, if(date_format(begdate,'%m/%d/%Y')='00/00/0000','',date_format(begdate,'%m/%d/%Y')) as begd8 from pharma_master where record_id = '$_GET[id]';"); 
		if($res['supplier'] != 0 || $res['supplier'] != "") { list($sname) = $con->getArray("select tradename from contact_info where file_id='$res[supplier]';"); }
	
		/* $pi = $con->getArray("select ifnull(sum(b.qty),0) from phy_header a left join phy_details b on a.doc_no = b.doc_no and a.branch = b.branch where a.branch = '$_SESSION[branchid]' and b.item_code = '$res[item_code]' and a.status = 'Finalized' and a.posting_date = '2018-05-26' GROUP BY b.item_code;");				
		$cur = $con->getArray("select sum(purchases+inbound-outbound-pullouts-sold) as currentbalance from ibook where item_code = '$res[item_code]' and doc_date between '2018-05-26' and '".date('Y-m-d')."' and doc_branch = '$_SESSION[branchid]';");
		...
		*/
		...
		$pi = $res['begqty'];
		list($sold) = $con->getArray("SELECT SUM(b.qty) FROM pharma_so_header a LEFT JOIN pharma_so_details b ON a.so_no = b.so_no WHERE a.status = 'Finalized' AND a.so_date >= '$res[begdate]' AND b.code = '$res[item_code]';");
		list($purchases) = $con->getArray("SELECT SUM(purchases) FROM ibook WHERE item_code = '$res[item_code]' AND doc_type LIKE '%PHARMA%' and doc_date >= '$res[begdate]';");
		list($pullouts) = $con->getArray("SELECT SUM(pullouts) FROM ibook WHERE item_code = '$res[item_code]' AND doc_type LIKE '%PHARMA%' and doc_date >= '$res[begdate]';");

		list($userOff) = $con->getArray("select fullname from user_info where role= 'PHARMACY ASSISTANT';");
		

		$oh = $pi + $purchases - $sold - $pullouts;
...
<script>
	$(document).ready(function(){
		...
		$("#item_beginning_date, #expiry_d8").datepicker({changeMonth: true, changeYear: true });
	...
	function savePInfo(rid) {
		...
		if(confirm("Are you sure you want to save changes made to this product profile?") == true) {
			...
			if(msg!="") {
				...
				$.post("src/sjerp.php", { mod: "checkPharmaDupCode", rid: rid, item_code: $("#item_code").val(), barcode: $("#item_barcode").val(), sid: Math.random() }, function(data) {
					if(data == "NODUPLICATE") {
						var url = $(document.merchandise).serialize();
							url = "mod=savePharmaInfo&"+url;
						$.post("src/sjerp.php", url);
						alert("Merchandise Successfuly Saved!"); 
						parent.closeDialog("#itemdetails");	
						parent.showPharmaItems($("#item_code").val());
	...
	function deletePharma(rid) {
		if(confirm("Are you sure you want to delete this record?") == true) {
			$.post("src/sjerp.php", { mod: "deletePharma", rid: rid, sid: Math.random() }, function() { 
				alert("Merchandise Record Successfully Deleted!"); 
				parent.closeDialog("#itemdetails");	
				parent.showPharmaItems();
			});
		}
	}
	...
	function getMyCode() {
		$.post("src/sjerp.php", { mod: "getPharmaIcode", mid: $("#item_category").val(), sgroup: $("#item_sgroup").val(), sid: Math.random() }, function(data) {
			$("#item_code").val(data);
		});
	}
	
	function getSgroup(mgroup) {
		$.post("src/sjerp.php", { mod: "getPharmaSgroup", mgroup: mgroup, sid: Math.random() }, function(data) {
...
<form name="merchandise" id="merchandise">
			...
			<td align=left>
				<?php
					if($_GET['mod'] == 1) {
						...
						if($res['record_id'] != "") {
							echo '&nbsp;&nbsp;<a href="#" onClick="deletePharma(\'' . $_GET['id'] . '\');" class="topClickers"><img src="images/icons/delete.png" width=18 height=18 align=absmiddle />&nbsp;Delete Record</a>';
						}
					}
				?>
			</td>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" >
		<tr>
			...
			<td width="40%">
				<select name="item_category" id="item_category" style="width: 70%;" class="nInput" <?php if($res['record_id'] == '') { echo "onChange=\"javascript: getMyCode(); getSgroup(this.value);\""; } ?>>
					...
					<?php
						$mit = $con->dbquery("select id,`group` from pharma_mgroup;");
			...
			<td width="40%">
				<select name="item_sgroup" id="item_sgroup" style="width: 70%;" class="nInput" <?php if($res['record_id'] == '') { echo "onChange=\"javascript: getMyCode();\""; } ?>>
					...
					<?php
						...
						if($res['record_id'] != '') {
							$mit = $con->dbquery("select id, sgroup from pharma_sgroup where 1=1 and pid = '$res[category]' order by sgroup asc;");
							while(list($o,$oo) = $mit->fetch_array()) {
								echo "<option value='$o' ";
		...
		<tr>
			...
			<td width="40%" align=left>
				<input type="text" name="item_rack_no" id="item_rack_no" style="width: 70%; font-weight:bold;" class="nInput" value="<?php echo $res['rack_no']; ?>">
			</td>
		</tr>
		<tr><td height=1></td></tr>
		<tr>
			...
			<td width="40%" align=left>
				<input type="text" name="item_genericname" id="item_genericname" style="width: 80%;" class="nInput" value="<?php echo $res['generic_name']; ?>">
			</td>
			<td width="10%" align=left class="spandix-l">
				Label :
			</td>
			<td width="40%" align=left>
				<input type="text" name="item_level" id="item_level" style="width: 70%; font-weight:bold;" class="nInput" value="<?php echo $res['level']; ?>">
			</td>
		</tr>
	</table>
	...
	<table width="100%" cellpadding=0 cellspacing=1 class="td_content" style="padding:10px;" border=0>
		...
		<tr>
			...
			<td width=30%>
				<input type="text" name="srp" id="srp" style="width: 200px; text-align: right;" value='<?php echo number_format($res['srp'],2); ?>' class="nInput">
			</td>
		...
		<tr>
			<td width=15% class="spandix-l">Registered Barcode :</td>
			<td width=30%><input type="text" name="item_barcode" id="item_barcode" style="width: 200px; text-align: right;" value='<?php echo $res['barcode']; ?>' class="nInput"></td>
			<td width=20% class="spandix-l">Cost of Sales Account :</td>
			<td width=35% align=right>
		...
		<tr>
			<td width=15% class="spandix-l">Expiry Date :</td>
			<td width=30%>
				<input type="text" name="expiry_d8" id="expiry_d8" style="width: 200px; text-align: right;" value='<?php echo $res['expiry_d8']; ?>' class="nInput">
			</td>
			<td width=20% class="spandix-l">SC/PWD Discount :</td>
			<td width=35%>
		...
		<tr>
			<td colspan=4 width=100% align=left><hr width=100% align=left></hr></td>
		</tr>

		<tr>
			<td width=15% class="spandix-l" align=right></td>
			<td width=30% class="spandix-1" style="font-size: 12px;" align=right><b>Qty-Sold : &nbsp;&nbsp;</b><?php echo number_format($sold,2);?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			</td>
			<td width=55% class="spandix-l" colspan=2 align=right><b>Qty-On-Hand : &nbsp;&nbsp;</b><?php echo number_format($oh,2);?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Qty Available : &nbsp;&nbsp;</b><?php if($oh < 0) { echo "0"; } else { echo number_format($end,2); }?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Qty-On-PO : &nbsp;&nbsp;</b><?php echo number_format($onPO,2);?></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td width=55% colspan=2 align=right class="spandix-l">Inventory Beginning : <input type="text" name="item_beginning" id="item_beginning" style="width: 30%;text-align: right;" class="nInput" value="<?php echo number_format($res['begqty'],2); ?>"></td>
		</tr>
		<tr>
			<td width=15% class="spandix-l"></td>
			<td width=30%>
			...
			<td width=55% colspan=2 align=right class="spandix-l">Posting Date of Inventory Beginning : <input type="text" name="item_beginning_date" id="item_beginning_date" style="width: 30%;text-align: right;" class="nInput" value="<?php echo $res['begd8']; ?>"></td>
		</tr>
		<tr>
			<td width=15% class="spandix-l"></td>
			<td width=30%></td>
