<?php
	...
	switch($_POST['mod']) {
		...
		case "addItem":
			$p->dbquery("INSERT INTO po_details (branch,po_no,costcenter,item_code,description,qty,unit,cost,amount,trace_no) VALUES ('$bid','$_POST[po_no]','$_POST[costcenter]','$_POST[item]','".$p->escapeString(htmlentities($_POST['description']))."','".$p->formatDigit($_POST['qty'])."','$_POST[unit]','".$p->formatDigit($_POST['cost'])."','".$p->formatDigit($_POST['amount'])."','$_POST[trace_no]');");
		break;

		case "updateItem":
			$p->dbquery("UPDATE po_details set costcenter = '$_POST[costcenter]',item_code = '$_POST[item]',description='".$p->escapeString(htmlentities($_POST['description']))."',qty = '".$p->formatDigit($_POST['qty'])."',unit = '$_POST[unit]',cost = '".$p->formatDigit($_POST['cost'])."',amount = '".$p->formatDigit($_POST['amount'])."' where line_id = '$_POST[lid]';");
		...
		case "check4print":
			list($a) = $p->getArray("select count(*) from po_header where po_no = '$_POST[po_no]' and branch = '$bid';");
			list($b) = $p->getArray("select count(*) from po_details where po_no = '$_POST[po_no]' and branch = '$bid';");
		...
		case "getTotals":
			list($gross) = $p->getArray("SELECT SUM(ROUND(qty*cost,2)) AS gross FROM po_details WHERE po_no = '$_POST[po_no]' AND branch = '$bid';");
			if($gross == "") { $gross = "0.00"; }
		...
		case "retrieve":
			...
			$srrd = $p->dbquery("SELECT line_id AS id, a.costcenter, b.costcenter AS cc, description, item_code, unit, qty, cost, amount FROM po_details a LEFT JOIN options_costcenter b ON a.costcenter = b.unitcode WHERE trace_no = '$_REQUEST[trace_no]';");
