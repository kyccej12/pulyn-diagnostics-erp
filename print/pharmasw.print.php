...
/* MYSQL QUERIES SECTION */
	...
	$_ihead = $mydb->getArray("select trace_no, sw_no, lpad(sw_no,2,0) as rr, date_format(sw_date,'%m/%d/%Y') as d8, withdrawn_by, mr_no, ref_type, if(request_date!='0000-00-00',date_format(request_date,'%m/%d/%Y'),'') as rd8, amount, remarks from pharma_sw_header where sw_no = '$_REQUEST[sw_no]' and branch = '$_SESSION[branchid]';");
	$_idetails = $mydb->dbquery("select item_code, description, qty, unit, cost, amount from pharma_sw_details where sw_no = '$_REQUEST[sw_no]' and branch = '$_SESSION[branchid]';");
...
<td width="60%" align=right>
	<span style="font-weight: bold; font-size: 13pt; color: #000000;">STOCKS WITHDRAWAL SLIP (PHARMACY)</span><br />
	<barcode size=0.8 code="'.substr($bcode,0,18).'" type="C128A">
</td>
</tr>
</table>
