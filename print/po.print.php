...
/* MYSQL QUERIES SECTION */
	...
	$_ihead = $con->getArray("select po_no, lpad(po_no,8,0) as rr, date_format(po_date,'%m/%d/%Y') as d8, if(date_needed!='0000-00-00',date_format(date_needed,'%m/%d/%Y'),'') as nd8, delivery_address, supplier, supplier_name, supplier_addr, requested_by, amount, remarks, mrs_no,c.description as terms FROM po_header a LEFT JOIN contact_info b ON a.supplier = b.file_id LEFT JOIN options_terms c ON a.terms = c.terms_id where po_no='$_REQUEST[po_no]' and branch = '$_SESSION[branchid]';");
	$_idetails = $con->dbquery("select item_code, description, sum(qty) as qty, unit, cost, ROUND(sum(qty*cost),2) amount from po_details where po_no = '$_REQUEST[po_no]' and branch = '$_SESSION[branchid]' group by po_no,item_code;");
	$bcode = STR_PAD($_REQUEST['user'],2,'0',STR_PAD_LEFT)."-".$_ihead['po_no']."-".date('Ymd');
	
	list($dscnt) = $con->getArray("select sum(amount) from po_details where po_no = '$_REQUEST[po_no]' and branch = '$_SESSION[branchid]' and item_code = 'DSCT-001-001';");
	list($dl_charge) = $con->getArray("select sum(amount) from po_details where po_no = '$_REQUEST[po_no]' and branch = '$_SESSION[branchid]' and item_code = 'DLCH-001-001';");
	list($lbr_charge) = $con->getArray("select sum(amount) from po_details where po_no = '$_REQUEST[po_no]' and branch = '$_SESSION[branchid]' and item_code = 'LABR-001-001';");
	list($frgt_charge) = $con->getArray("select sum(amount) from po_details where po_no = '$_REQUEST[po_no]' and branch = '$_SESSION[branchid]' and item_code = 'FRGHT-001-001';");
	...
	<table width=100% cellpadding=5 style="border: 1px solid #000000;">
		<tr>
			...
			<td width=50% align=center><b>APPROVED BY:</b><br><br><br>_________________________________<br><font size=5>Management</font></td>
		</tr>
	</table>';
...
<tr>
	<td width="100%" align=center>
		<span style="font-weight: bold; font-size: 16pt; color: #000000; padding-top: 10px;">PURCHASE ORDER</span>
	</td>
</tr>
</table>
<br/><br/>
