...
/* MYSQL QUERIES SECTION */
	...
	$_ihead = $con->getArray("SELECT DATE_FORMAT(result_date,'%m/%d/%Y') AS rdate, b.patient_name, b.patient_address, c.gender as xgender, IF(c.gender='M','Male','Female') AS gender, c.gender as xgender, c.birthdate, b.physician, a.serialno,a.created_by,b.trace_no FROM lab_occultblood a LEFT JOIN so_header b ON a.so_no = b.so_no AND a.branch = b.branch LEFT JOIN patient_info c ON b.patient_id = c.patient_id WHERE a.so_no = '$_REQUEST[so_no]' and a.serialno = '$_REQUEST[serialno]' AND a.branch = '$_SESSION[branchid]';");  
    $b = $con->getArray("SELECT color,consistency,result,remarks,verified_by,verified FROM lab_occultblood WHERE so_no = '$_REQUEST[so_no]' and branch = '$_SESSION[branchid]' and serialno = '$_REQUEST[serialno]';");
...
<div id="main">
    ...
    <table width=80% cellpadding=0 cellspacing=0 align=center style="border-collapse: collapse; font-size: 15px;">
            ...
            <tr>
                <td class="itemHeader">COLOR</td>
                <td class="itemHeader">CONSISTENCY</td>
				<td class="itemHeader">OCCULT BLOOD</td>

            </tr>
            <tr>
                <td class="itemResult">'.$b['color'].'</td>
				<td class="itemResult">'.$b['consistency'].'</td>
