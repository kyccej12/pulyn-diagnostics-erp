...
/* MYSQL QUERIES SECTION */
	...
	$mpdf->SetAuthor("OMDC Prime Medical Diagnostics");


	$mpdf->SetDisplayMode(50);

    if($_GET['cid'] != '') {
		$fs .= " and c.customer_code = '$_GET[cid]' ";
	}

	$outerQuery = $con->dbquery("SELECT a.so_no, a.pid, c.customer_code, c.customer_name, CONCAT(b.lname,', ',b.fname,' ',b.mname) AS pname, b.birthdate, b.gender, b.street, b.brgy, b.city, b.province,b.employer, a.examined_by, evaluated_by FROM peme a LEFT JOIN patient_info b ON a.pid = b.patient_id LEFT JOIN so_header c ON a.pid = c.patient_id WHERE a.so_date BETWEEN '$dtf' AND '$dt2' AND a.examined_by > 0 AND a.evaluated_by > 0 $fs GROUP BY a.so_no ORDER BY b.lname, b.fname, b.mname;");	
...
$_ihead = $con->getArray("SELECT a.*, LPAD(c.so_no,6,0) AS sono, DATE_FORMAT(c.so_date,'%m/%d/%Y') AS d8, DATE_FORMAT(a.updated_on, '%m/%d/%Y') AS examin_d8, b.patient_id, fname, b.lname, b.mname,b.suffix,FLOOR(DATEDIFF(c.so_date,b.birthdate)/364.25) AS age,DATE_FORMAT(b.birthdate,'%M %d') AS date1, DATE_FORMAT(b.birthdate,'%Y') AS date2, classification,examined_by,evaluated_by,b.gender,b.cstat,b.brgy,b.city,b.province,b.street,c.customer_name,c.customer_address FROM peme a LEFT JOIN omdc.patient_info b ON a.pid = b.patient_id LEFT JOIN so_header c ON a.so_no = c.so_no AND a.pid = c.patient_id WHERE a.so_no= '$outerRow[so_no]' AND a.pid= '$outerRow[pid]';");
$a = $con->dbquery("SELECT LPAD(so_no,6,0) AS sono, pid,CONCAT(c.fname,' ',c.lname) AS pname, pm_history FROM peme a LEFT JOIN options_medicalhistory b ON b.id = a.pid LEFT JOIN patient_info c ON a.pid = c.patient_id WHERE a.so_no= '$outerRow[so_no]' AND a.pid= '$outerRow[pid]';");
$b = $con->getArray("SELECT * FROM peme WHERE so_no= '$outerRow[so_no]' AND pid= '$outerRow[pid]';");
$c = $con->getArray("SELECT pid,examined_by,DATE_FORMAT(examined_on,'%m/%d/%Y') AS examin_d8,TIME_FORMAT(examined_on,'%h:%m:%s') AS examin_tym,evaluated_by,DATE_FORMAT(evaluated_on,'%m/%d/%Y') AS eval_d8, TIME_FORMAT(evaluated_on,'%h:%m:%s') AS eval_tym FROM peme WHERE so_no = '$outerRow[so_no]' and pid = '$outerRow[pid]';");
