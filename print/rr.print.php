...
/* MYSQL QUERIES SECTION */
	...
	$_ihead = $p->getArray("select trace_no, lpad(rr_no,6,0) as rr, date_format(rr_date,'%m/%d/%Y') as d8, supplier, supplier_name, supplier_addr, invoice_no, date_format(invoice_date,'%m/%d/%Y') as id8, received_by, amount, remarks from rr_header where rr_no='$_REQUEST[rr_no]' and branch = '$_SESSION[branchid]';");
	$_idetails = $p->dbquery("select costcenter, if(po_no='','--',lpad(po_no,6,0)) as po_no, if(po_date='0000-00-00','--',date_format(po_date,'%m/%d/%y')) as po_date, item_code, description, qty, unit, cost, amount from rr_details where rr_no = '$_REQUEST[rr_no]' and branch = '$_SESSION[branchid]';");
