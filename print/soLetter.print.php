...
/* MYSQL QUERIES SECTION */
	...
	$_ihead = $con->getArray("SELECT *, LPAD(so_no,6,0) AS sono, DATE_FORMAT(a.created_on, '%m/%d/%Y %h:%i:%s %p') AS dprocessed, DATE_FORMAT(so_date,'%m/%d/%Y') AS d8, IF(loa_date!='0000-00-00',DATE_FORMAT(loa_date,'%m/%d/%Y'),'') AS load8, IF(hmo_card_expiry!='0000-00-00',DATE_FORMAT(hmo_card_expiry,'%m/%d/%Y'),'') AS exd8, LPAD(patient_id,6,'0') AS pid, IF(customer_code!=0,CONCAT('[',LPAD(customer_code,6,'0'),'] ',customer_name),CONCAT(patient_name,' (Patient)')) AS cname, IF(customer_code!=0,customer_address,patient_address) AS customer_address, b.description AS terms_desc,c.fullname AS created_by FROM so_header a LEFT JOIN options_terms b ON a.terms = b.terms_id LEFT JOIN user_info c ON a.created_by = c.emp_id where so_no = '$_REQUEST[so_no]' and branch = '$_SESSION[branchid]';");
	...
	$_p = $con->getArray("SELECT DATE_FORMAT(birthdate,'%m/%d/%Y') AS bday, birthdate, IF(gender='M','Male','Female') AS gender, b.civil_status, mobile_no, email_add FROM pddmc.patient_info a LEFT JOIN omdcpayroll.options_civilstatus b ON a.cstat = b.csid WHERE a.patient_id = '$_ihead[patient_id]';");
...
$mpdf=new mPDF('win-1252','Letter','','',10,10,102,30,10,10);
...
<table width="100%" cellpadding=0 cellspaing=0>
	<tr>
		...
		<td style="color:#000000; padding-top: 5px;" valign=top>
			<b>'.$co['company_name'].'</b><br/><span style="font-size: 8pt;">'.$co['company_address'].'<br/>Tel # '.$co['tel_no'].'<br/>'.$co['website'].'<br/>NON-VAT REG. TIN: '.$bit['tin_no'].'</span>
		</td>
...
<table width=100% cellpadding=2 cellspacing=0 style="font-size: 11px;margin-top:10px;">
	<tr>
...
mpdf-->
	...
	<table class="items" width="100%" style="font-size: 11px; border-collapse: collapse;" cellpadding="3">
		<thead>
			<tr>
