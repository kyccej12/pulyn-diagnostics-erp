...
/* MYSQL QUERIES SECTION */
	...
	$_ihead = $mydb->getArray("select trace_no, srr_no, lpad(srr_no,6,0) as srr, date_format(srr_date,'%m/%d/%Y') as d8, received_from, received_by, ref_type, ref_no, if(ref_date!='0000-00-00',date_format(ref_date,'%m/%d/%Y'),'') as rd8, amount, remarks from srr_header where srr_no = '$_REQUEST[srr_no]' and branch = '$_SESSION[branchid]';");
	$_idetails = $mydb->dbquery("select item_code, description, qty, unit, cost, amount from srr_details where srr_no = '$_REQUEST[srr_no]' and branch = '$_SESSION[branchid]';");
	$bcode = $_ihead['trace_no'];
	
/* END OF SQL QUERIES */

$mpdf=new mPDF('win-1252','FOLIO-H','','',15,15,65,20,10,10);
...
<b>Received From :</b><br /><br /><i>'.$_ihead['received_from'].'</i></b><br/><br/><b>Received By:</b><br/><br/><i>'.$_ihead['received_by'].'</i></td>
...
<tr>
	...
	<td width=34% align=center><b>APPROVED BY:</b><br><br>_________________________________<br><font size=3>Signature over Printed Name</font></td>
</tr>
</table>
<table width=100%>
