<?php
	...
	if(isset($_REQUEST['queryString']) && $_REQUEST['queryString']!= "") {
		...
		$r = mysql_query("select item_code,description,unit_cost,unit from products_master where (locate('$_REQUEST[queryString]',description) > 0 or locate('$_REQUEST[queryString]',barcode) or locate('$_REQUEST[queryString]',item_code)) and `group` in ('RM','TG') limit 10");
			$i = 0;
