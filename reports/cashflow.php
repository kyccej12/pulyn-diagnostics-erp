<?php
	...
	/* MYSQL QUERIES SECTION */
		...
		$adesc = $con->getAcctDesc($_GET['source'],$_SESSION['company']);
		list($run) = $con->getArray("SELECT SUM(debit-credit) FROM acctg_gl WHERE acct = '$_GET[source]' AND doc_date < '".formatDate($_GET['dtf'])."' AND acct = '$_GET[source]' and company = '$_SESSION[company]' and branch = '$_SESSION[branchid]' group by acct;");
		$query = $con->dbquery("SELECT cy, doc_no, CONCAT(cy,'-',LPAD(doc_no,6,0)) AS dno, DATE_FORMAT(doc_date,'%m/%d/%Y') AS dd8, doc_type, debit, credit, debit-credit AS xamt, doc_remarks AS remarks FROM acctg_gl WHERE company = '$_SESSION[company]' and branch = '$_SESSION[branchid]' and doc_date BETWEEN '".formatDate($_GET['dtf'])."' AND '".formatDate($_GET['dt2'])."' AND acct = '$_GET[source]' ORDER BY doc_date ASC;");
		$str = "SELECT cy, doc_no, CONCAT(cy,'-',LPAD(doc_no,6,0)) AS dno, DATE_FORMAT(doc_date,'%m/%d/%Y') AS dd8, doc_type, debit, credit, debit-credit AS xamt, doc_remarks AS remarks FROM acctg_gl WHERE company = '$_SESSION[company]' and branch = '$_SESSION[branchid]' and doc_date BETWEEN '".formatDate($_GET['dtf'])."' AND '".formatDate($_GET['dt2'])."' AND acct = '$_GET[source]' ORDER BY doc_date ASC;";
...
<table width="100%">
	<tr>
		<td style="color:#000000;" width=13%><img src="../images/'.$co['headerlogo'].'" height=70 /></td>
...
<thead>
	<tr>
		...
		<td width="7%" align=left><b>CHECK #</b></td>
		<td width="8%" align=left><b>CHECK DATE</b></td>
		<td width="6%" align=right><b>DEBIT</b></td>
		<td width="6%" align=right><b>CREDIT</b></td>
		<td width="10%" align=right><b>RUNNING BALANCE</b></td>
		<td width="15%" align=left><b>MEMO</b></td>
	</tr>
</thead>
<tbody>
...
while($row = $query->fetch_array()) {
	switch($row['doc_type']) {
		case "CV":
			list($payee,$check_no,$check_date) = $con->getArray("select concat('(',payee,') ',payee_name) as payee, check_no, if(check_date!='0000-00-00',date_format(check_date,'%m/%d/%Y'),'') as check_date from cv_header where cv_no = '$row[doc_no]' and company = '$_SESSION[company]' and branch = '$_SESSION[branchid]';");
		break;
		default :
