<?php
	...
	for($i = 0; $i <= 11; $i++) {
		...
		list($amt) = $con->getArray("select ROUND(sum(b.qty*cost),2) from po_header a inner join po_details b on a.company = b.company and a.branch = b.branch and a.po_no = b.po_no where po_date between '$dtf' and '$dt2' and `status` = 'Finalized' AND a.company = '$_SESSION[company]' AND a.branch = '$_SESSION[branchid]';");
		list($amt2) = $con->getArray("select ROUND(sum(b.qty*cost),2) from po_header a inner join po_details b on a.company = b.company and a.branch = b.branch and a.po_no = b.po_no where po_date between '$ly_dtf' and '$ly_dtf' and `status` = 'Finalized' AND a.company = '$_SESSION[company]' AND a.branch = '$_SESSION[branchid]';");
