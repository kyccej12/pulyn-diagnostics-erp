<?php
	...
	for($i = 0; $i <= 11; $i++) {
		...
		list($amt) = $con->getArray("select sum(credit-debit) from acctg_gl a left join acctg_accounts b ON a.acct = b.acct_code where doc_date between '$dtf' and '$dt2' and b.acct_grp in ('9','10') AND a.branch = '$_SESSION[branchid]';");
		list($amt2) = $con->getArray("select sum(credit-debit) from acctg_gl a left join acctg_accounts b ON a.acct = b.acct_code where doc_date between '$ly_dtf' and '$ly_dtf' and b.acct_grp in ('9','10') AND a.branch = '$_SESSION[branchid]';");
