<?php // content="text/plain; charset=utf-8"
	...
	$a = $con->dbquery("select * from (SELECT c.proj_name, c.proj_code, ROUND(SUM(b.qty*b.cost),2) AS amount FROM po_header a INNER JOIN po_details b ON a.company=b.company AND a.branch = b.branch AND a.po_no = b.po_no INNER JOIN options_project c ON a.proj_name = c.proj_id WHERE a.status = 'Finalized' AND po_date BETWEEN '".formatDate($_GET['dtf'])."' AND '".formatDate($_GET['dt2'])."' GROUP BY a.proj_name) a order by amount desc;");
	while($b = $a->fetch_array()) {
		
		$perc = ROUND(($b[2]/$t)*100,2);
		
		$data[$i] = $b[2];
		$lbl[$i] = "$b[0] ($perc%)";
		$legend[$i] = "(".$b[1].") ".number_format($b[2],2);
		$i++;
	}
