<?php
	...
	/* MYSQL QUERIES SECTION */
		...
		$query = $mydb->dbquery("SELECT 'ex' AS `type`,a.so_no, a.branch, DATE_FORMAT(a.so_date,'%m/%d/%Y') AS so_date, a.clinic AS clinic_no, a.procedure, b.customer_code, b.customer_name, a.pid AS patient_id, b.patient_name, CONCAT(b.terms,' ','Days') AS terms, a.code, a.examined_by, CONCAT(c.fullname,', ',c.prefix) AS examiner, a.evaluated_by, CONCAT(d.fullname,', ',d.prefix) AS evaluator, e.description, e.unit_price FROM peme a LEFT JOIN so_header b ON a.so_no = b.so_no LEFT JOIN options_doctors c ON a.examined_by = c.id LEFT JOIN options_doctors d ON a.evaluated_by = d.id LEFT JOIN services_master e ON a.code = e.code WHERE 1=1 AND a.so_date BETWEEN '$dtf' AND '$dt2' and a.examined_by = '$_GET[cid]' UNION SELECT 'ev' AS `type`,a.so_no, a.branch, DATE_FORMAT(a.so_date,'%m/%d/%Y') AS so_date, a.clinic AS clinic_no, a.procedure, b.customer_code, b.customer_name, a.pid AS patient_id, b.patient_name, CONCAT(b.terms,' ','Days') AS terms, a.code, a.examined_by, CONCAT(c.fullname,', ',c.prefix) AS examiner, a.evaluated_by, CONCAT(d.fullname,', ',d.prefix) AS evaluator, e.description, e.unit_price FROM peme a LEFT JOIN so_header b ON a.so_no = b.so_no LEFT JOIN options_doctors c ON a.examined_by = c.id LEFT JOIN options_doctors d ON a.evaluated_by = d.id LEFT JOIN services_master e ON a.code = e.code WHERE 1=1 AND a.so_date BETWEEN '$dtf' AND '$dt2' and a.evaluated_by = '$_GET[cid]' ORDER BY so_no;");
		list($examintotal) = $mydb->getArray("SELECT COUNT(*) FROM peme WHERE so_date BETWEEN '$dtf' AND '$dt2' AND examined_by = '$_GET[cid]';");
		list($evaltotal) = $mydb->getArray("SELECT COUNT(*) FROM peme WHERE so_date BETWEEN '$dtf' AND '$dt2' AND evaluated_by = '$_GET[cid]';");
		/* END OF SQL QUERIES */
...
<table width="100%">
	<tr>
		...
		<td width="40%" align=right>
			<span style="font-weight: bold; font-size: 8pt; color: #000000;">PEME DOCTORS TALLY REPORT</span><br/>Date Covered ' . $_GET['dtf'] . ' - ' . $_GET['dt2'] .'</span>
		</td>
	</tr>
</table>
</htmlpageheader>
...
while($row = $query->fetch_array()) {
	...
	$html = $html . '<tr bgcolor="'.$mydb->initBackground($i).'">
		...
		<td align=center>'. $row['examiner'] .'</td>
		<td align=center>'. $row['evaluator'] .'</td>
...
$html = $html . '<tr bgcolor="'.$mydb->initBackground($i).'">
				...
				<td align=center><b>' . number_format($examintotal) . '</b></td>
				<td align=center><b>' . number_format($evaltotal) . '</b></td>
				<td align=right><b>' .number_format($amountGT,2) . '</b></td>
				</tr>

				<tr bgcolor="'.$mydb->initBackground($i).'">
					<td colspan=11 align=left><b>TOTAL PATIENT EXAMINED/EVALUATED:</b>&nbsp;&nbsp;<b>'. number_format($total) .'</b></td>
				</tr>';
