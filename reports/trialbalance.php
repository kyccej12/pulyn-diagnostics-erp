...
<table width="100%">
	<tr>
		...
		<td width="40%" align=right>
			<span style="font-weight: bold; font-size: 10pt; color: #000000;">TRIAL BALANCE CUMMULATIVE</span><br /><span style="font-size: 6pt; font-style: italic;"><b>' . $myBranch .'</b></span><br /><span style="font-size: 6pt; font-style: italic;">Date As Of ' . $_GET['asof'] . '</span>
		</td>
	</tr>
</table>
</htmlpageheader>
...
while($row = $query->fetch_array(MYSQLI_BOTH)) {
	if($row['amt'] != 0) {
		if($row['amt'] > 0) { $db = $row['amt']; $cr = 0; } else { $db = 0; $cr = $row['amt']*-1; }
		$html = $html . '<tr>
