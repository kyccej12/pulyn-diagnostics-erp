...
<script>
	...
	$(document).ready(function() {
		var myTable = $('#itemlist').DataTable({
			...
			"bProcessing": true,
			"responsive": true,
			"sAjaxSource": "data/samplelist.php?displayPending=<?php echo $_REQUEST['displayPending']; ?>",
			"scroller": true,
			"order": [[ 1, "desc" ]],
	...
	function writeResult() {
		...
		if(!lid) {
			...
			if(status == 'Cancelled') { msg = "It appears that a result is already \"Cancelled\" Sales Order!"; }
			...
			<tr>
				<td>
					...
					<button class="ui-button ui-widget ui-corner-all" onClick="refreshList();">
						<span class="ui-icon ui-icon-refresh"></span> Display Current Request
					</button>
					<button class="ui-button ui-widget ui-corner-all" onClick="displayPending();">
						<span class="ui-icon ui-icon-copy"></span> Display Previous Requests
					</button>
...
<form name="frmDisplayPending" id="frmDisplayPending" action="samples.list.php" method="POST">
