<?php
	...
	switch($_REQUEST['mod']) {
		...
		case "addItem":
			list($isE) = $con->getArray("select count(*) from sw_details where trace_no = '$_POST[trace_no]' and item_code = '$_POST[item]' and branch = '$_SESSION[branchid]';");
			if($isE > 0) {
				$s = "update ignore sw_details set qty = qty + ".$con->formatDigit($_POST['qty']).", amount = amount + ".$con->formatDigit($_POST['amount'])." where trace_no = '$_POST[trace_no]' and item_code = '$_POST[item]' and branch = '$_SESSION[branchid]';";
			} else {
				$s = "insert ignore into sw_details (branch,sw_no,item_code,description,qty,unit,cost,amount,trace_no) values ('$_SESSION[branchid]','$_POST[sw_no]','$_POST[item]','".$con->escapeString(htmlentities($_POST['description']))."','".$con->formatDigit($_POST['qty'])."','$_POST[unit]','".$con->formatDigit($_POST['cost'])."','".$con->formatDigit($_POST['amount'])."','$_POST[trace_no]');";
			}
		...
		case "retrieveLine":
			echo json_encode($con->getArray("select *,format(cost,2) as ucost, format(amount,2) as amt from sw_details where line_id = '$_POST[lid]';"));
		break;

		case "updateItem":
			$con->dbquery("update sw_details set item_code = '$_POST[item]', description = '".$con->escapeString(htmlentities($_POST['description']))."', qty = '".$con->formatDigit($_POST['qty'])."', unit = '$_POST[unit]',cost = '".$con->formatDigit($_POST['cost'])."', amount = '".$con->formatDigit($_POST['amount'])."' where line_id = '$_POST[lid]';");
			updateAmount($_POST['sw_no']);
		break;
		...
		case "finalizeSW":
			...
			while($ibook = $iquery->fetch_array()) {
				$con->dbquery("INSERT IGNORE INTO ibook (doc_no,doc_date,doc_type,doc_branch,cname,item_code,uom,pullouts,posted_by,posted_on) VALUES ('$ibook[doc_no]','$ibook[doc_date]','SW','$ibook[branch]','".$con->escapeString($ibook['customer'])."','$ibook[item_code]','$ibook[unit]','$ibook[qty]','$_SESSION[userid]',now());");
			}
		break;
		case "reopenSW":
		...
		case "retrieve":
			...
			$query = $con->dbquery("SELECT line_id as id, description, item_code, unit, qty, cost, amount FROM sw_details where trace_no = '$_REQUEST[trace_no]';");
